...
import { NepalTrekPathDisplayProps, NepalTrekPathProps } from './Map';
...
interface NepalTrekPathDisplayPropsWithElevation {
    id: number;
    properties: NepalTrekPathProps;
    path: Array<{ location: LatLng, elevation: number, resolution: number }>;
}

async function getElevation(elevator: any, path: LatLng[], retries = 5) {
    console.log("Path Length", path.length);
    return new Promise<Array<{ elevation: number, location: LatLng, resolution: number }>>((resolve, reject) => {
        elevator.getElevationAlongPath({
            'path': path,
            'samples': 256
        }, (elevs: Array<{ elevation: number, location: LatLng, resolution: number }>, status: string) => {
            if (status === 'OK') {
                resolve(elevs);
            } else {
                reject(status);
...
async function requestRetry<T>(action: () => Promise<T>, maxRetries = 5, sleepMs = 0) {
    let numRetries = 0;
    ...
    while (numRetries < maxRetries) {
        try {
            ...
            console.log(`Error: ${e}`, `Retrying (${numRetries}/${maxRetries})`);
            numRetries++;
            await sleep();
        }
    }
    throw Error('Max number of retries reached');
}

function useElevationService(paths: NepalTrekPathDisplayProps[] = []) {
    const [result, setResult] = React.useState<NepalTrekPathDisplayPropsWithElevation[]>([]);
        ...
        async function fetchElevations() {
            if (window.google && window.google.maps && window.google.maps.ElevationService) {
                try {
                    ...
                    // Map paths to elevation requests and wait for all to finish
                    const elevations = await Promise.all(
                        paths.map(async ({ path, ...restPath }) => {
                            try {
                                const elevs = await requestRetry(() => getElevation(elevator, path), 5, 5000);
                                return {
                                    path: elevs,
                                    ...restPath
                                };
                    ...
                    setIsLoading(false);
                    setResult(elevations);
                    console.log("ELEVATIONS", elevations, error);
                } catch (error) {
                    setIsLoading(false);
                    setError(error);
                }
            } else {
                setError(Error("Necessary API's not accessible"))
            }
        }
...
//     let pathsWithElevation: NepalTrekPathDisplayPropsWithElevation[] = [];
//     if (window.google && window.google.maps && window.google.maps.ElevationService) {
//         pathsWithElevation = await useMemo(async () => {
//             const elevator = new window.google.maps.ElevationService();
//             // Map paths to elevation requests
//             return await Promise.all(
//                 paths.map(({ path, ...restPath }) =>
//                     new Promise<NepalTrekPathDisplayPropsWithElevation>((resolve, reject) => {
//                         elevator.getElevationAlongPath({
//                             'path': path,
//                             'samples': 256
//                         }, (elevs: Array<{ elevation: number, location: LatLng, resolution: number }>, status) => {
//                             if (status === 'OK') {
//                                 resolve({
//                                     ...restPath,
//                                     path: elevs
...
//     return [pathsWithElevation, error];
// }

interface ElevationGraphProps {
    currentDay?: number;
    percentDayComplete?: number;
    paths: NepalTrekPathDisplayProps[];
    className?: string;
}
...
const ElevationGraph: React.SFC<ElevationGraphProps> = ({ paths = [], className, currentDay, percentDayComplete }) => {
    const [elevations, isLoading, error] = useElevationService(paths);
    ...
    if (error) {
        return (
            <div>Error with elevation service</div>
        );
    }
    // const data = paths.map(({ properties: { styleUrl, styleHash, ...restProps } }) => (restProps))
    const data = [
        { 'altitude': 100, 'distance': 1 },
        { 'altitude': 200, 'distance': 2 },
        { 'altitude': 300, 'distance': 4 },
    ];
